...
#define ESPERA 5
	...
	void preenche(Matriz matriz){
	  ...
	  linha_max=matriz.linhas/4;
	  
	  if (t==0) {
	      int j,i;
	      for (j=0;j<linha_max;j++){
			      for (i=0;i<matriz.colunas;i++) {
				  x=x+(matriz.conteudo[i][j]*matriz.conteudo[j][i]);
				  matriz.prodint=x;
			      }
	      }
	  }
	  if (t==1) {
	      int j,i;
	      for (j=linha_max;j<linha_max*2;j++){
			      for (i=0;i<matriz.colunas;i++) {
				  x=x+(matriz.conteudo[i][j]*matriz.conteudo[j][i]);
				  matriz.prodint=x;
			      }
	  ...
	  if (t==2) {
	      int j,i;
	      for (j=linha_max*2;j<linha_max*3;j++){
			      for (i=0;i<matriz.colunas;i++) {
				  x=x+(matriz.conteudo[i][j]*matriz.conteudo[j][i]);
				  matriz.prodint=x;
			      }
	  ...
	  if (t==3) {
	      int j,i;
	      for (j=linha_max*3;j<matriz.linhas;j++){
			      for (i=0;i<matriz.colunas;i++) {
				  x=x+(matriz.conteudo[i][j]*matriz.conteudo[j][i]);
				  matriz.prodint=x;
			      }
...
int main(void){
		...
		for(t=0; t<4; t++){ 
			printf("Criando thread numero %d\n", t); 
